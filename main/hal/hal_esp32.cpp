...
void HalEsp32::init()
{
    ...
    // Rotate to landscape using LVGL software rotation
    lv_display_set_rotation(lvDisp, LV_DISPLAY_ROTATION_90);
    bsp_display_backlight_on();

    // Removed initial color test overlay
    ...
    mclog::tagInfo(_tag, "wifi module init");
    // Enable power to ESP32-C6 WiFi module
    bsp_set_wifi_power_enable(true);
    delay(100);  // Give ESP32-C6 time to power up
    
    // Initialize WiFi (will start AP mode if CONFIG_TAB5_WIFI_REMOTE_ENABLE is set)
    wifi_init();
