...
esp_err_t bsp_cam_osc_init(void)
{
    ...
    // Use high-frequency PLL-div clock to generate 24MHz camera MCLK reliably
    timer_conf.clk_cfg         = LEDC_USE_PLL_DIV_CLK;
    ...
    ch_conf.duty       = 1;
    ch_conf.hpoint     = 0;
    // Camera MCLK does not need to keep alive across sleep; avoid LEDC warning
...
esp_err_t bsp_audio_init(const i2s_std_config_t* i2s_config)
{
    ...
    chan_cfg.dma_frame_num     = 256;  // Aligned to 64 bytes for DMA to fix alignment warning
...
{
    esp_err_t ret = ESP_OK;
    // Enforce 48 kHz to avoid duplex rate conflict with RX path
    if (rate != 48000) {
        ESP_LOGD(TAG, "Forcing playback sample_rate from %u to 48000 to match RX", (unsigned)rate);
        rate = 48000;
    }
...
#define LCD_LEDC_CH LEDC_CHANNEL_1  // Use channel 1 as configured in sdkconfig
esp_err_t bsp_display_brightness_init(void)
{
    // Reset GPIO 22 first to clear any existing reservations
    ESP_LOGI(TAG, "Resetting GPIO %d before LEDC configuration", BSP_LCD_BACKLIGHT);
    gpio_reset_pin(BSP_LCD_BACKLIGHT);
    
    // Short delay to ensure GPIO reset is complete
    vTaskDelay(pdMS_TO_TICKS(10));
    
    // Configure GPIO 22 as output before LEDC configuration
    ...
    // Setup LEDC peripheral for PWM backlight control (using separate timer from camera MCLK)
    const ledc_timer_config_t lcd_backlight_timer = {
        .speed_mode      = LEDC_LOW_SPEED_MODE,
        .duty_resolution = LEDC_TIMER_10_BIT, // 1024 steps
        .timer_num       = LEDC_TIMER_1,      // Use timer 1 (camera uses timer 0)
        .freq_hz         = 20000, // 20 kHz to avoid visible PWM flicker
    ...
    const ledc_channel_config_t lcd_backlight_channel = {
        ...
        .timer_sel  = LEDC_TIMER_1,           // Use timer 1 to match the timer above
        .duty       = 0,
        .hpoint     = 0,
...
esp_err_t bsp_display_brightness_set(int brightness_percent)
{
    ...
    uint32_t duty_cycle = (1023 * brightness_percent) / 100; // 10-bit resolution => 0..1023
...
esp_err_t bsp_display_new_with_handles(const bsp_display_config_t* config, bsp_lcd_handles_t* ret_handles)
{
    ...
    /* Initialize I2C and IO expander for LCD reset control */
    ESP_RETURN_ON_ERROR(bsp_i2c_init(), TAG, "I2C init failed");
    bsp_io_expander_pi4ioe_init(bsp_i2c_get_handle());
    
    /* Reset display before DSI communication */
    ESP_LOGI(TAG, "Resetting display controller");
    bsp_reset_tp(); // This also resets the LCD via IO expander
    vTaskDelay(pdMS_TO_TICKS(100)); // Allow display to stabilize
    ...
    /* Allow other tasks to run during DSI initialization */
    vTaskDelay(pdMS_TO_TICKS(10));
    ...
    /* Allow other tasks to run before panel IO creation */
    vTaskDelay(pdMS_TO_TICKS(10));
    ESP_LOGI(TAG, "Creating DSI panel IO interface...");
    ...
    esp_lcd_dpi_panel_config_t dpi_config = {
        ...
        .dpi_clock_freq_mhz = 60,  // revert to stable clock
...
lv_display_t* bsp_display_start_with_config(const bsp_display_cfg_t* cfg)
{
    ...
    // Initialize brightness/GPIO configuration before LVGL port init
